...
void readSaleCars(string fileName, SaleCar cars[], int &count)
{
	...
	while ( getline(fin,plateNumber,',') ){
		...
		Contact con;
		con.setOwner(contactName);
		con.setPhone(contactPhone);
		cars[count].setContact(con);
		count++;
		count++;
	}
	fin.close();
...
int main()
{
	...
	readSaleCars("sale_car.txt",saleCars,nSaleCars);
	...
	readRentalCars("rental_mpv.txt",mpvRentalCars,mpvCount);	
	...
	readRentalCars("rental_regular.txt",regularRentalCars,regularCount);
	...
	int a = searchSaleCar(plateNumber,saleCars,nSaleCars); 
    int b = searchRentalCar(plateNumber,mpvRentalCars,mpvCount);
    int c = searchRentalCar(plateNumber,regularRentalCars,regularCount);

	if(a != -1)
	{
		cout<<"The car is in the list of cars for sale."<<endl;
		cout<<"Model :"<<saleCars[a].getModel()<<endl;
		cout<<"Owner :"<<saleCars[a].getContact().getOwner()<<endl;
	}	
	
	else if(b != -1)
	{
		cout<<"The car is in the list of mpv cars for rental."<<endl;
		mpvRentalCars[b].print();
	}
	
	else if(c != -1)
	{
		cout<<"The car is in the list of regular cars for rental."<<endl;
		regularRentalCars[c].print();
	}
